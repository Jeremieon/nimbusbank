...
async def get_current_user(authorization: str | None = Header(default=None)) -> dict:
    ...
    auth-service's JWKS — the correct check. Proves the caller presented a
    genuine, unexpired token; says nothing about their role. See require_admin
    below and the BFLA gap on GET /admin/overview in main.py."""
...
async def require_admin(current: dict = Depends(get_current_user)) -> dict:
    ...
    never applied to GET /admin/overview in main.py. Same "defined-but-not-
    applied" pattern as transfers-service and cards-service (API Security:
