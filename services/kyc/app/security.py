...
async def get_current_user(authorization: str | None = Header(default=None)) -> dict:
    ...
    JWKS. This is the CORRECT check — copied verbatim from accounts-service:
    RS256 signature verification *and* expiry (`exp`) enforcement, since
    jwt.decode validates `exp` by default.
    ...
    the resource they're requesting, nor about their role — see require_admin
    below (defined, then deliberately not applied) and the BOLA/BFLA gaps in
    main.py.
    """
...
async def require_admin(current: dict = Depends(get_current_user)) -> dict:
    """A real role check — a compliance officer / admin gate, defined and ready
    to use — and then, deliberately, NOT used by GET /kyc/pending or
    POST /kyc/{id}/review in main.py. That's the whole gap: read the two
    dependencies side by side and the missing piece is obvious (API Security:
    Broken Function Level Authorization). Same "defined but never applied"
    stance as transfers-service's require_admin."""
